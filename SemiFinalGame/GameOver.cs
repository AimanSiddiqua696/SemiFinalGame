...
namespace SemiFinalGame
{
    ...
    {
        public GameOver(int score, int coins, int lives)
        {
            InitializeComponent();
        ...
        protected override void OnResize(EventArgs e)
        {
            ...
            if (btnTryAgain != null && btnExit != null)
            {
                int gap = 20;
                int totalWidth = btnTryAgain.Width + btnExit.Width + gap;
                int startX = (this.ClientSize.Width - totalWidth) / 2;
                int y = this.ClientSize.Height - btnTryAgain.Height - 50; 

                btnTryAgain.Location = new Point(startX, y);
                btnExit.Location = new Point(startX + btnTryAgain.Width + gap, y);
            }
        }
